          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Cell Phone/Device</TableCell>
                <TableCell>Looking Away</TableCell>
                <TableCell>Eye Tracking</TableCell>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((log, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.cellPhoneCount + (log.prohibitedObjectCount || 0))}
                        label={log.cellPhoneCount + (log.prohibitedObjectCount || 0)}
                        color={getViolationColor(log.cellPhoneCount + (log.prohibitedObjectCount || 0))}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.lookingAwayCount)}
                        label={log.lookingAwayCount}
                        color={getViolationColor(log.lookingAwayCount)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.eyeTrackingCount)}
                        label={log.eyeTrackingCount}
                        color={getViolationColor(log.eyeTrackingCount)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.speechDetectionCount)}
                        label={log.speechDetectionCount}
                        color={getViolationColor(log.speechDetectionCount)}
                        size="small"
                      />
                    </TableCell>
                    <TableCell>
                      <Chip
                        icon={getViolationIcon(log.switchTabCount)}
                        label={log.switchTabCount || 0}
                        color={getViolationColor(log.switchTabCount || 0)}
                        size="small"
                      />
                    </TableCell>
                  </TableRow>
